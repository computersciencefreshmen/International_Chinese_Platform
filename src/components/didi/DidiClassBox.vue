...
const openDigitalHuman = () => {
  router.push('/student/digitalHuman')
}
...
<template>
  <div class="bg-white rounded-lg shadow-lg p-6 w-full">
    ...
    <div
      class="relative w-full h-64 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl overflow-hidden border-2 border-dashed border-gray-200 mb-6"
    >
      <!-- Connection Line -->
      <div
        ref="connectionLine"
      ...
      >
        ...
        />
        <div
          class="absolute -bottom-2 left-1/2 transform -translate-x-1/2 bg-blue-500 text-white text-xs px-2 py-0.5 rounded-full whitespace-nowrap"
        >
          学生
        </div>
      </div>
      ...
      >
        ...
        />
        <div
          class="absolute -bottom-2 left-1/2 transform -translate-x-1/2 bg-red-500 text-white text-xs px-2 py-0.5 rounded-full whitespace-nowrap"
        >
          老师
        </div>
      </div>
    ...
    <div class="flex justify-center space-x-4">
      ...
      <button
        ...
        class="px-6 py-3 bg-indigo-500 hover:bg-indigo-600 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50"
      >
        数字人课堂
      </button>
    </div>
    ...
    <!-- Connection Info -->
    <div
      v-if="connectionState === '已连接'"
      class="mt-6 p-4 bg-green-50 rounded-lg border border-green-200 text-center"
    >
    ...
    <div v-if="studentPersonStore.appointmentList.length" class="mt-8">
      ...
      <div
        v-for="(item, idx) in studentPersonStore.appointmentList"
        :key="idx"
        class="relative group bg-gradient-to-br from-blue-100/80 to-blue-50/80 rounded-2xl p-6 mb-6 shadow-xl border-2 border-blue-200/60 flex flex-col md:flex-row md:items-center md:justify-between transition-transform duration-300 hover:-translate-y-2 hover:shadow-2xl overflow-hidden"
      >
        <!-- 渐变立体边框 -->
        <div
          class="absolute inset-0 rounded-2xl pointer-events-none border-4 border-transparent group-hover:border-blue-300 group-hover:shadow-lg transition-all duration-300 z-0"
        ></div>
        <!-- 左侧内容 -->
        <div class="flex-1 flex flex-col md:flex-row md:items-center z-10">
          ...
          <div class="flex-shrink-0 flex flex-col items-center mr-6">
            <img
              :src="studentAvatar"
              class="w-16 h-16 rounded-full border-4 border-white shadow-md object-cover mb-2"
              alt="预约人"
            />
          ...
          <div class="flex-1 space-y-2">
            <div
              class="flex items-center text-base text-gray-700 font-semibold"
            >
              <svg
            ...
            <div class="flex items-center text-base text-gray-700">
              ...
              <span class="text-blue-500">选择话轮：</span
              >{{ item.selectedRound }}
            </div>
            <div class="flex items-center text-base text-gray-700">
              <svg
              ...
              <span class="text-blue-500">预约时间：</span
              >{{ formatDateToChinese(item.appointmentTime) }}
              <span class="ml-3 text-xs text-green-600 font-bold">{{
                getRelativeTime(item.appointmentTime)
              }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-col items-center mt-6 md:mt-0 md:ml-8 z-10">
          <img
            :src="item.teacherAvatar || teacherAvatar"
            class="w-14 h-14 rounded-full border-2 border-blue-300 shadow object-cover mb-2"
            alt="老师"
          />
          <span class="text-xs text-gray-500 mb-1">老师</span>
          <span
            class="inline-block bg-blue-400 text-white text-xs px-4 py-1 rounded-full shadow mb-2"
            >待上课</span
          >
          <div class="flex items-center text-xs text-gray-400">
            ...
            预约创建于：{{
              formatDateToChinese(item.createdAt || item.appointmentTime)
            }}
          </div>
        </div>
      </div>
