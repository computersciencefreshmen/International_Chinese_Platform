<script setup>
import { ref, nextTick, watch, onUnmounted, onMounted, computed } from 'vue' //引入vue的计算属性
...
const { send, status } = useWebSocket({
  url: computed(() => {
    const token = studentStore.getUserInfo()?.token || ''
    return `ws://localhost:7788/websocket?token=${token}`
  }),
  onMessage: (event) => {
    try {
      console.log(event)
