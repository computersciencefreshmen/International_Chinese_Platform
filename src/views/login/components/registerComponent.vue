...
const emailType = ref('')
const fullEmail = () => {
  const input = RegisterFormData.value.email.trim()
  return input.includes('@') ? input : `${input}${emailType.value}`
}
...
const handleSendCode = () => {
  ...
  if (!RegisterFormData.value.email.trim()) {
    errors.value.email = '邮箱不能为空'
  } else if (!RegisterFormData.value.email.includes('@') && !emailType.value) {
    errors.value.email = '请选择邮箱域名'
  } else if (!/^\S+@\S+\.\S+$/.test(email)) {
    errors.value.email = '请输入有效邮箱'
  } else if (!isRegisterActive.value) {
    ElMessage.warning('请先选择注册身份')
  }

  isSendCode.value = Boolean(
    !errors.value.email && isRegisterActive.value && email
  )
}

//注册时发送邮箱验证码
const handleSendEmail = async () => {
  if (!isSendCode.value || isSendingCode.value) return

  isSendingCode.value = true
  try {
    const response = await sendEmail(fullEmail(), isRegisterActive.value)
    const developmentCode = response.data?.data?.developmentCode

    if (developmentCode) {
      RegisterFormData.value.code = String(developmentCode)
      ElMessage.success('验证码已发送，本地演示码已填入')
    } else {
    ...
    if (!error.messageShown) {
      ElMessage.error(
        error.response?.data?.msg || error.message || '验证码发送失败'
      )
    }
  } finally {
...
const verifyInput = () => {
  ...
  // 校验验证码
  if (!/^\d{6}$/.test(RegisterFormData.value.code.trim())) {
    errors.value.code = '请输入 6 位验证码'
  }
  ...
  if (!RegisterFormData.value.password) {
    ...
    !/[A-Za-z]/.test(RegisterFormData.value.password) ||
    !/\d/.test(RegisterFormData.value.password)
  ) {
    errors.value.password = '密码至少 8 位，且需包含字母和数字'
  }
...
const verifyMemberInfo = () => {
  MemberError.value.name = ''
  MemberError.value.nationality = ''
  MemberError.value.region = ''
  MemberError.value.age = ''
  ...
  if (!getMemberInfo.value.nationality) {
    MemberError.value.nationality = '国籍不能为空'
  }
  ...
  if (
    ...
    getMemberInfo.value.age < 6 ||
    getMemberInfo.value.age > 120
  ) {
    MemberError.value.age = '请输入 6 至 120 岁之间的年龄'
  }
...
const handleMemberInfo = () => {
  ...
  //执行学生信息逻辑
  if (isRegisterActive.value === 'student') {
    step.value.stepStudent = true // 显示学生注册
  } else if (isRegisterActive.value === 'teacher') {
...
const selectedChineseLevel = () => {
  if (isActiveStudentLevel.value === 6) return 'beginner'
  return studentChineseLevel.value[isActiveStudentLevel.value]?.level ?? null
}

const selectedTeacherCertificates = () =>
  teacherCertificate.value.flatMap((certificate) => {
    const level = certificate.levels?.[certificate.isActiveAgree]
    return level ? [`${certificate.name}（${level}）`] : []
  })

// 最后一步统一提交学生/教师注册。成功响应已通过 HttpOnly Cookie 建立会话。
const handleMemberEnter = async () => {
  if (isSubmitting.value || !isRegisterActive.value) return

  isSubmitting.value = true
  try {
    const profile = await userStore.register({
      email: fullEmail(),
      password: RegisterFormData.value.password,
      verificationCode: RegisterFormData.value.code,
      role: isRegisterActive.value,
      displayName: getMemberInfo.value.name.trim(),
      country: getMemberInfo.value.nationality.trim(),
      region: getMemberInfo.value.region.trim(),
      age: Number(getMemberInfo.value.age),
      chineseLevel:
        isRegisterActive.value === 'student' ? selectedChineseLevel() : null,
      certificates:
        isRegisterActive.value === 'teacher'
          ? selectedTeacherCertificates()
          : undefined
    })
    ...
    await router.replace(roleRoutes[profile.role])
  } catch (error) {
    if (!error.messageShown) {
      ElMessage.error(
        error.response?.data?.msg || error.message || '注册失败，请稍后重试'
      )
    }
  } finally {
    ...
    <transition name="slide" mode="out-in">
      ...
      >
        <div class="flex items-center mt-4">
          <optionsCp
            text="我是学生"
            @click="selectRegistrationRole('student')"
            :isActive="isRegisterActive === 'student'"
          >
            <img
              class="w-10 h-10"
          ...
          <optionsCp
            text="我是老师"
            @click="selectRegistrationRole('teacher')"
            :isActive="isRegisterActive === 'teacher'"
          >
            <img
              class="w-12 h-12"
      ...
      <!-- 学生注册页面 -->
      <div
        v-else-if="isRegisterActive === 'student' && step.stepStudent"
        key="step-student"
      >
        <logoComponent class="scale-75 -ml-28"></logoComponent>
      ...
      <!-- 老师注册页面 -->
      <div
        v-else-if="isRegisterActive === 'teacher' && step.stepTeacher"
        key="step-teacher"
        class="py-4 px-6"
      >
        <logoComponent class="scale-75 -ml-28"></logoComponent>
      ...
      <div v-else-if="step.enterPlatform" key="step-enterPlatform">
        ...
        <div class="my-20 text-3xl text-center font-sans">
          <p class="leading-relaxed">{{ getMemberInfo.name }}，欢迎你</p>
